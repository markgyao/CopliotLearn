...
// Editable Variables
$grade = '08'; // Set the grade here
$subject = 'math'; // Set the subject here (e.g., 'math', 'science')
$schoolType = 'middle_school'; // Set the school type here (e.g., 'middle_school', 'high_school', 'elementary_school')

// Set the file paths
$directory = '/opt/cplearn/data/prek_to_12/' . $schoolType . '/grade_' . $grade; // The server path to the files
$webDirectory = '/data/prek_to_12/' . $schoolType . '/grade_' . $grade . '/' . $subject; // The web accessible path
$backPage = 'admin_grade_' . $grade . '.php'; // Back button page link

$message = "";

// Check if the directory exists before scanning it
if (!is_dir($directory)) {
...
if ($files === false) {
    die("Failed to open directory: " . $directory);
}

$files = array_diff($files, array('.', '..')); // Remove '.' and '..' from file listing
...
<head>
    ...
    <title>Grade <?php echo $grade; ?> - <?php echo ucfirst($subject); ?> Directory Listing</title>
    <style>
        body {
...
<body>
    <div class="file-list-container" id="fileListContainer">
        <?php echo $message; ?>
        <h2>Grade <?php echo $grade; ?> - <?php echo ucfirst($subject); ?></h2>
        <table>
            <tr>
                <th>File Name</th>
                <th>Actions</th>
    ...
    <script>
        function previewFile(filePath) {
            ...
            filePreview.data = filePath;
            document.getElementById('pdfDownloadLink').href = filePath;
            document.getElementById('previewContainer').classList.add('visible');
        }

        function printFile(filePath) {
            var previewWindow = window.open(filePath, '_blank');
            previewWindow.print();
        }
    </script>
</body>
